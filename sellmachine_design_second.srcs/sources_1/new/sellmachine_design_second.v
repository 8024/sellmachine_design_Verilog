...
module sellmachine_design_second(
    ...
    reg judge;
    light_press display(clk,charge_val,light,light_part);  
    ...
    if(Reset==0)
     ...
     else if((mealy_state==S1)&&coin_putin&&coin_val&&(mealy_nextstate==S1))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S1)&&buy_confirm&&(mealy_nextstate==S1))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S2)&&coin_putin&&(coin_val==0)&&(mealy_nextstate==S3))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S2)&&buy_confirm&&(mealy_nextstate==S2))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S3)&&buy_confirm&&goods_choose&&(mealy_nextstate==S3))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S3)&&buy_confirm&&(goods_choose==0)&&(mealy_nextstate==S0))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=1;          //取饮料
                charge_ind=1;           //找零
     ...
     else if((mealy_state==S3)&&coin_putin&&coin_val&&(mealy_nextstate==S3))
            begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S4)&&coin_putin&&coin_val&&(mealy_nextstate==S4))
             begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S5)&&coin_putin&&(mealy_nextstate==S5))
              begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
     ...
     else if((mealy_state==S6)&&coin_putin&&(mealy_nextstate==S6))
             begin
                hold_ind=1;             //机器被占用
                drinktk_ind=0;          //无法取饮料
                charge_ind=0;           //没有找零
    ...
    always @(posedge ensure)
        begin
           case(mealy_state)
                ...
                S2:begin
                   if(cancel_flag)          //付完2元后取消操作，返回S7状态
                    begin
                    mealy_nextstate = S0;
                    end
                else if(coin_putin)         //继续投币         
               ...
               S3:begin
                ...
                S4:begin
                   if(cancel_flag)          //付了4元后取消操作，返回S7状态
                    begin
                    mealy_nextstate = S0;
                    end
                else if(coin_putin)         //继续投币
                ...
                S5:begin
                  ...
                  else if(buy_confirm)      //点击购买，返回S7状态
                    begin
                    mealy_nextstate = S0;                   
                    end
                ...
                S6:begin
                    if(cancel_flag)     //付了10元，点击取消操作按钮，返钱，返回S7状态
                    begin
                    mealy_nextstate = S0;                   
                    end 
                 else if(buy_confirm)
